    ...
    thirdParty: true      // 第三方
        ...
        function renderFriends() {
            ...
            AppState.friendGroups.forEach(group => {
                ...
                groupHeader.style.cssText = 'padding:8px 15px;font-size:12px;color:#999;font-weight:600;background:#f9f9f9;cursor:pointer;display:flex;justify-content:space-between;align-items:center;user-select:none;';
                groupHeader.dataset.groupId = group.id;
                groupHeader.dataset.collapsed = isCollapsed;
                
                groupHeader.innerHTML = `
                    <div style="flex:1;display:flex;align-items:center;gap:8px;">
                        <span style="display:inline-block;width:12px;text-align:center;transition:transform 0.2s;transform:${isCollapsed ? 'rotate(-90deg)' : 'rotate(0deg)'};">▼</span>
                        <span>${group.name}</span>
                        <span style="margin-left:4px;">(${groupFriends.length})</span>
                    </div>
                    <div style="display:flex;gap:4px;">
                        <button onclick="event.stopPropagation();editFriendGroup('${group.id}')" style="background:none;border:none;color:#666;cursor:pointer;padding:0;font-size:12px;">编辑</button>
                        ${group.id !== 'group_default' ? `<button onclick="event.stopPropagation();deleteFriendGroup('${group.id}')" style="background:none;border:none;color:#f44;cursor:pointer;padding:0;font-size:12px;">删除</button>` : ''}
                    </div>
                `;
                ...
                groupFriends.forEach(friend => {
                    ...
                    item.innerHTML = `
                        <div class="friend-item-content" style="display:flex;align-items:center;gap:12px;padding:10px 15px;background:#fff;position:relative;z-index:2;margin-left:12px;">
...
大多数情况下仍应使用普通单条回复，多消息模式用于特定场景。`);
        ...
        function parseThinkingProcess(text) {
            ...
            // 检查是否包含思考过程标记
            if (!text.includes('[THINK]') && !text.includes('[REPLY')) {
                return null;  // 没有思考过程，返回null表示普通消息
            }
            ...
            // 如果有思考内容但没有回复，创建默认回复
            if (thinkingContent && messages.length === 0) {
                messages.push({
                    type: 'reply',
                    content: '（思考中...）',
                    delay: 0
        ...
        function openChatMoreMenu(chat) {
            ...
            menu.innerHTML = `
                <div class="chat-menu-item" onclick="openCharacterSettings(AppState.currentChat);" style="padding:12px 16px;border-bottom:1px solid #f0f0f0;cursor:pointer;font-size:13px;transition:background 0.15s;">角色设置</div>
                <div class="chat-menu-item" onclick="manualSummarizeConversation();" style="padding:12px 16px;border-bottom:1px solid #f0f0f0;cursor:pointer;font-size:13px;transition:background 0.15s;">手动总结</div>
                <div class="chat-menu-item" onclick="openContextSummarySettings();" style="padding:12px 16px;cursor:pointer;font-size:13px;transition:background 0.15s;">总结设置</div>
            `;
            ...
            document.addEventListener('click', (e) => {
                if (!e.target.closest('#chat-more-menu') && !e.target.closest('.chat-more')) {
                    if (menu) menu.remove();
                }
            }, { once: true });
        }
        ...
        function manualSummarizeConversation() {
            ...
            // 调用API生成总结
            summarizeContextWithAPI(AppState.currentChat.id);
        }

        function summarizeContextWithAPI(convId) {
            if (!AppState.apiSettings.endpoint || !AppState.apiSettings.apiKey || !AppState.apiSettings.selectedModel) {
                showToast('请先配置API设置');
                return;
            }
                ...
                <div class="emoji-mgmt-content" style="max-width:500px;max-height:90vh;overflow-y:auto;">
                    ...
                    <div style="padding:16px;">
                        ...
                        <div style="display:flex;gap:8px;justify-content:center;border-top:1px solid #e8e8e8;padding-top:16px;">
                            ...
                            <button onclick="showSummaryHistory('${chat.id}');" style="padding:8px 16px;border:1px solid #0066cc;border-radius:4px;background:#fff;color:#0066cc;cursor:pointer;font-size:13px;flex:1;">查看总结</button>
        ...
        function showCharacterMindHistory(charId) {
            ...
            for (let i = chat.mindStates.length - 1; i >= 0; i--) {
                ...
                content += `
                    <div style="margin-bottom:16px;padding:12px;background:#f9f9f9;border-radius:4px;border-left:3px solid #333;display:flex;justify-content:space-between;align-items:flex-start;">
                        <div style="flex:1;">
                        ...
                        <button onclick="deleteSingleMindState('${chat.id}', ${i})" style="margin-left:8px;padding:4px 8px;border:1px solid #FF3B30;background:#fff;color:#FF3B30;border-radius:4px;cursor:pointer;font-size:11px;flex-shrink:0;">删除</button>
                    </div>
                `;
            }
