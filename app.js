    ...
    game: true            // 游戏中心
        ...
        function saveToStorage() {
            try {
                ...
                localStorage.setItem('shupianjAppState', JSON.stringify(stateToDump));
            } catch (e) {
...
记住：表情包是情绪表达工具，图片是视觉内容；处理时方式完全不同。`);
        ...
        function initNotificationSystem() {
            ...
            // 移动端触摸事件
            notificationBar.addEventListener('touchstart', function() {
                pauseNotificationAutoHide();
            });

            notificationBar.addEventListener('touchend', function() {
                resumeNotificationAutoHide();
            });
        }
        ...
        function showNotification(data) {
            ...
            document.getElementById('notification-preview').textContent = data.message;
            ...
            // 移除旧的动画类
            bar.classList.remove('hide', 'slide-out');
        ...
        function triggerNotificationIfLeftChat(convId) {
            ...
            // 构建通知数据
            const lastMessage = AppState.messages[convId] ? 
                AppState.messages[convId][AppState.messages[convId].length - 1] : 
                null;
        ...
        window.getNotificationStatus = function() {
            const bar = document.getElementById('notification-bar');
            console.log('通知栏DOM:', bar ? '✅ 存在' : '❌ 不存在');
            console.log('通知栏类名:', bar?.className);
            console.log('当前通知数据:', AppState.notification.current);
            console.log('自动隐藏计时器:', AppState.notification.autoHideTimer ? '⏱️ 运行中' : '⏹️ 未运行');
        };
